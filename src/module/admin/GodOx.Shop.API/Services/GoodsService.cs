...
namespace GodOx.Shop.API.Services
{
    ...
    public class GoodsService : BaseServer<Goods>, IGoodsService
    {
        ...
        public async Task<ApiResult> GetListPageAsync(KeyListTenantQuery query)
        {
            ...
            return new ApiResult(data: new { count = datas.TotalItems, items = datas.Items });
        }
        ...
        public async Task<ApiResult> AddAsync(GoodsInput input)
        {
            try
            {
                ...
                var goodsId = await Db.Insertable(goods).ExecuteReturnIdentityAsync();
                // 保存规格
        ...
        private async Task DealwithGoodsSpec(int goodsId, GoodsInput input)
        {
            ...
            if (input.SpecType == SpecTypeEnum.Single.GetValue<int>())
            {
                ...
                if (null == goodsSpec)
                {
                    throw new ArgumentNullException("商品规格实体数据不能为空！");
                }
            ...
            else
            {
                ...
                {
                    throw new ArgumentNullException("商品规格实体数据集合不能为空！");
                }
                ...
                {
                    throw new ArgumentNullException("商品规格实体关系集合数据不能为空！");
                }
                //根据规格值反推规格组id
        ...
        public async Task<ApiResult> ModifyAsync(GoodsModifyInput input)
        {
            ...
            if (goods == null) throw new ArgumentNullException($"此商品{input.Id}没有查找对应的商品信息");
            try
            {
                Db.BeginTran();
                // 更新商品
