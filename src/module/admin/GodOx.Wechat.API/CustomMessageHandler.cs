...
namespace GodOx.Wechat.API
{
    ...
    public class CustomMessageHandler : MessageHandler<DefaultMpMessageContext>
    {
        ...
        public override async Task<IResponseMessageBase> OnTextOrEventRequestAsync(RequestMessageText requestMessage)
        {
            ...
            await Senparc.Weixin.MP.AdvancedAPIs.CustomApi.SendTextAsync(Senparc.Weixin.Config.SenparcWeixinSetting.MpSetting.WeixinAppId, OpenId, $"这是一条异步的客服消息");//注意：只有测试号或部署到正式环境的正式服务号可用此接口
